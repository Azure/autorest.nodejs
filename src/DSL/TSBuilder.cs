...
namespace AutoRest.NodeJS.DSL
{
    ...
    public class TSBuilder : JSBuilder
    {
        ...
        public void ExportInterface(string interfaceName, Action<TSInterface> action)
        {
            ExportInterface(interfaceName, null, action);
        }

        public void ExportInterface(string interfaceName, string extendedInterfaceName, Action<TSInterface> action)
        {
            Block($"export interface {interfaceName}{(string.IsNullOrEmpty(extendedInterfaceName) ? "" : $" extends {extendedInterfaceName}")}", block =>
            {
