...
namespace AutoRest.NodeJS
{
    public static class ExtensionsLoader
    {
        public static IAnyPlugin GetPlugin(string name)
        {
            switch (name)
            {
                case "NodeJS": return new AutoRest.NodeJS.PluginJs();
                case "Azure.NodeJS": return new AutoRest.NodeJS.Azure.PluginJsa();
            }
    ...
    public class Program : NewPlugin
    {
        public static int Main(string[] args )
        {
            if(args != null && args.Length > 0 && args[0] == "--server") {
                ...
                connection.Dispatch<IEnumerable<string>>("GetPluginNames", async () => new []{ "nodejs" });
